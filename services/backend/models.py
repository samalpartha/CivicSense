...
class QueryContext(BaseModel):
    ...
    language: Optional[str] = Field(
        default="en", description="Preferred language for response", example="en"
    )
...
class QueryResponse(BaseModel):
    ...
    category: str = Field(
        ..., description="Classified category of the query", example="education"
    )
    severity: str = Field(
        ...,
...
class HealthResponse(BaseModel):
    ...
    status: str = Field(..., description="Overall service status", example="healthy")
    kafka_connected: bool = Field(
        ..., description="Whether Kafka consumer is connected and running", example=True
    )
    active_connections: int = Field(
        ..., description="Number of active WebSocket connections", example=5, ge=0
    )
...
class RootResponse(BaseModel):
    ...
    service: str = Field(..., description="Service name", example="CivicSense Backend")
...
class ErrorResponse(BaseModel):
    ...
    class Config:
        json_schema_extra = {"example": {"detail": "Message is required"}}
...
class WebSocketQueryMessage(BaseModel):
    ...
    type: str = Field(
        ..., description="Message type", example="query", pattern="^query$"
    )
    message: str = Field(
        ...,
...
class WebSocketResponseMessage(BaseModel):
    ...
    type: str = Field(..., description="Message type", example="response")
    message: str = Field(
        ...,
    ...
    severity: Optional[str] = Field(
        default="info", description="Severity level", example="low"
    )
    affected_areas: Optional[List[str]] = Field(
        default=[], description="Affected geographic areas", example=["Downtown"]
    )
    timestamp: Optional[str] = Field(
        default=None, description="Response timestamp", example="2025-12-25T10:30:00"
    )
...
class WebSocketStatusMessage(BaseModel):
    ...
    type: str = Field(
        ..., description="Message type", example="status", pattern="^status$"
    )
    message: str = Field(
        ..., description="Status message", example="Processing your question..."
    )

    class Config:
        json_schema_extra = {
            "example": {"type": "status", "message": "Processing your question..."}
        }
...
class WebSocketErrorMessage(BaseModel):
    ...
    type: str = Field(
        ..., description="Message type", example="error", pattern="^error$"
    )
    message: str = Field(
        ..., description="Error message", example="Unable to process request"
    )
...
class WebSocketSystemMessage(BaseModel):
    ...
    type: str = Field(
        ..., description="Message type", example="system", pattern="^system$"
    )
    message: str = Field(
        ..., description="System message", example="Connected to CivicSense"
    )
