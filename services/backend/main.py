...
tags_metadata = [
    {
        "name": "Health",
        "description": "System health monitoring and liveness probes."
    },
    {
        "name": "Query",
    ...
    },
    {
        ...
        "description": "User profile and preference settings."
    }
]
...
async def get_weather(lat: float = 40.7128, lon: float = -74.0060, city: str = "New York"):
    global weather_cache
    current_time = time.time()
    
    # Check cache (simple implementation: cache by city for demo)
    if weather_cache["data"] and weather_cache["city"] == city and (current_time - weather_cache["timestamp"] < WEATHER_TTL):
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            elif wmo_code in [51, 53, 55, 56, 57, 61, 63, 65]: condition = "Rainy"
...
async def update_flink_stats():
    ...
    try:
        async for stats in flink_consumer.consume_aggregates():
            ...
            logger.debug(f"Flink stats updated: {stats['events_today']} events, {stats['active_alerts']} alerts")
    except Exception as e:
...
async def acknowledge_alert(alert_id: str, action: AlertActionRequest):
    return {"status": "success", "alert_id": alert_id, "state": "acknowledged", "timestamp": "2025-12-26T20:00:00Z"}
...
async def dismiss_alert(alert_id: str, action: AlertActionRequest):
    return {"status": "success", "alert_id": alert_id, "state": "dismissed", "timestamp": "2025-12-26T20:00:00Z"}
