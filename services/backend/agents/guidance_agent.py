...
Keep responses under 250 words unless complexity requires more."""
    ...
    async def generate(
        ...
        try:
            ...
            logger.info(f"Guidance generated for {category} query ({len(response)} chars)")
            ...
            fallback_answer = self._generate_fallback_response(query, category, impact, user_type)
            return {
